...
즉, `PrismaPg`는 PrismaClient와 PostgreSQL 연결 Pool 사이를 이어주는 중간 계층이다.
...
`PrismaModule`과 `PrismaService`가 단순한 DB 설정 파일이 아니라, NestJS 애플리케이션의 생명주기 안에서 DB 연결을 관리하는 구조이다.
